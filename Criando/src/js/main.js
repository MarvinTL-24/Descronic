...
function createSelectWorldEl(a,i) {
    ...
    i2.setAttribute('src', `src/assets/classes/${a}/${a}_logo.png`)
...
function creatorCharacterSelectorEl(a, i, c) {
    ...
    const i1 = document.createElement('img')
    i1.setAttribute('src', `src/assets/classes/${a}/tokens/${a}_${data[i]['tokens'][c]}`)
...
function setContent(a, i, c=0) {
    contentImg.setAttribute('src', `src/assets/classes/${a}/${a}_logo.png`)
...
function changeChar(a, i, c) {
    ...
    setTimeout(() => {
        contentH2.innerText = data[i]['name'][c]
        contentP.innerText = data[i]['desc'][c]  
        viewBg.style.background = `url(src/assets/classes/${a}/wpp/${a}_${data[i]['wpp'][c]})`
...
for(let index = 0; index < data.length; index++) {
    ...
    el.addEventListener('click', () => {
        ...
        viewBg.style.background = `url(src/assets/classes/${a}/wpp/${a}_${data[index]['wpp'][0]})`
        viewBg.style.backgroundSize = 'cover'
        viewBg.style.backgroundPosition = 'center'
